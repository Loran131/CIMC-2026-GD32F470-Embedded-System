...
#define NORFLASH_CS_GPIO_PORT           GPIOD
#define NORFLASH_CS_GPIO_PIN            GPIO_PIN_6
#define NORFLASH_CS_GPIO_CLK            RCU_GPIOD     /* GPIODÊ±ÖÓÊ¹ÄÜ */
...
#define BY25Q256    0X6818          /* BY25Q256 Ð¾Æ¬ID */
#define NM25Q64     0X5216          /* NM25Q64  Ð¾Æ¬ID */
#define NM25Q128    0X5217          /* NM25Q128 Ð¾Æ¬ID */
#define GD25Q40     0xC813
														
extern uint16_t g_norflash_type;    /* ¶¨ÒåFLASHÐ¾Æ¬ÐÍºÅ */
...
void norflash_write_disable(void);                                           /* Ð´±£»¤ */
...
void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen);    /* Ð´Èëpage */
void norflash_write_nocheck(uint8_t *pbuf, uint32_t addr, uint32_t datalen); /* Ð´flash,²»´ø²Á³ý */

void norflash_erase_chip(void);                                              /* ÕûÆ¬²Á³ý */
void norflash_erase_sector(uint32_t saddr);                                  /* ÉÈÇø²Á³ý */
void norflash_read(uint8_t *pbuf, uint32_t addr, uint32_t datalen);          /* ¶ÁÈ¡flash */
void norflash_write(uint8_t *pbuf, uint32_t addr, uint32_t datalen);         /* Ð´Èëflash */
