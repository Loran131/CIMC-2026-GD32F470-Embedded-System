...
static unsigned char bin_buf[256];
static int ascii_wait = 0;
static unsigned char high_nibble;
...
/**
 * @brief 解析一帧原始数据（A5 B6 开头，B6 A5 结尾，含CRC校验）
 * @param raw  原始数据缓冲区
 * @param len  数据长度
 * @param frame 输出解析结果
 * @return 0成功, -1失败
...
{
    // 最小长度：帧头2 + 设备ID2 + 帧类型1 + 命令字2 + 版本1 + 内容长度1 + 内容0 + CRC2 + 帧尾2 = 13
    if (len < 13) return -1;
