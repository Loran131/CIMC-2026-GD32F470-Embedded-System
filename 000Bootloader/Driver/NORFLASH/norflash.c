...
uint16_t g_norflash_type = GD25Q40;     
...
void norflash_init(void)
{
    ...
    rcu_periph_clock_enable(NORFLASH_CS_GPIO_CLK);    /* NORFLASH CS脚 时钟使能 */

    /* NORFLASH_CS引脚模式设置 推挽输出 */
    gpio_mode_set(NORFLASH_CS_GPIO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, NORFLASH_CS_GPIO_PIN);
    gpio_output_options_set(NORFLASH_CS_GPIO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_MAX, NORFLASH_CS_GPIO_PIN);

    NORFLASH_CS(1);                       /* NORFLASH_CS拉高，取消片选 */

    spi0_init();                          /* 初始化SPI0 */
    spi0_set_speed(SPI_SPEED_2);          /* SPI0 切换到高速状态 60Mhz */
...
{
    NORFLASH_CS(0);
    spi0_read_write_byte(FLASH_WriteEnable);   /* 发送写使能指令 */
    NORFLASH_CS(1);
}
...
{
    NORFLASH_CS(0);
    spi0_read_write_byte(FLASH_WriteDisable);  /* 发送写禁止指令 */
    NORFLASH_CS(1);
}
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    spi0_read_write_byte(FLASH_ManufactDeviceID);   /* 发送读ID 命令 */
...
void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    for (i = 0; i < datalen; i++)
    {
        spi0_read_write_byte(pbuf[i]);         /* 循环写入 */
    }
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    spi0_read_write_byte(FLASH_ChipErase);  /* 发送全片擦除命令 */ 
    NORFLASH_CS(1);
